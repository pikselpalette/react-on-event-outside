...
const eventDistributor = (eventType: string, e: Event, ...args) => callbackInstances[eventType].forEach((instance) => {
  const node: Node | null = ReactDOM.findDOMNode(instance); // eslint-disable-line react/no-find-dom-node
  ...
  componentDidMount() {
    this.events.forEach((event) => {
      ...
      if (!callbackInstances[event].length) {
        eventListeners[event] = eventListeners[event] || ((...args) => eventDistributor(event, ...args));
