...
type Props = {
  interactableComponentRef: React.Ref,
  on: {
    [string]: (e: Event) => void
  },
  children: React.Node
  ...
  componentDidMount() {
    this.events.forEach((event) => {
      ...
      if (!callbackInstances[event].length) {
        eventListeners[event] = eventListeners[event] || ((...args) => eventDistributor(this.props.interactableComponentRef.current, event, ...args));
